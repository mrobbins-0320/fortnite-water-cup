...
const newPlayer = (name = "") => ({
  ...
  matches: Array.from({ length: NUM_MATCHES }, () => ({
    placement: "", elims: 0, reboots: 0, teamWipes: 0, waterShots: 0,
  })),
});
...
const calcPlayer = (player) => {
  ...
  player.matches.forEach((m) => {
    if (m.placement !== "") pts += PLACEMENT_PTS(m.placement);
...
const calcTeam = (team) => {
  ...
  const teamPlacementPts = team.matchPlacements.reduce(
    (sum, p) => sum + (p !== "" ? PLACEMENT_PTS(p) : 0), 0
  );
...
// ─── Palette ──────────────────────────────────────────────────────────────────
const C = {
  bg:"#0a0e1a", card:"#111827", deep:"#0d1526", border:"#1e2d45",
  accent:"#00d4ff", purple:"#a855f7", danger:"#ff4d6d", water:"#60a5fa",
  gold:"#ffd700", green:"#34d399", text:"#e2e8f0", muted:"#64748b",
};
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
  ...
  .hero::before { content:''; position:absolute; inset:0; background:radial-gradient(ellipse at 50% 0%, rgba(0,212,255,.13) 0%, transparent 70%); pointer-events:none; }
  .trophy { font-size:2.2rem; margin-bottom:5px; filter:drop-shadow(0 0 12px #ffd70099); animation:bob 3s ease-in-out infinite; }
  @keyframes bob { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-7px)} }
  .title { font-family:'Bebas Neue',sans-serif; font-size:clamp(2rem,8vw,3.5rem); letter-spacing:.06em; line-height:1; background:linear-gradient(135deg,#ffd700 0%,#00d4ff 55%,#a855f7 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
  .subtitle { color:${C.muted}; font-size:.82rem; letter-spacing:.22em; text-transform:uppercase; margin-top:4px; }
  .sync-bar { display:flex; align-items:center; justify-content:center; gap:6px; font-size:.7rem; padding:5px 0 2px; }
  .sync-dot { width:6px; height:6px; border-radius:50%; }
  .sync-live { background:#34d399; box-shadow:0 0 6px #34d39988; animation:pulse 2s infinite; }
  .sync-off  { background:${C.muted}; }
  ...
  .sb-team { flex:1; min-width:130px; border-radius:9px; padding:12px 14px 16px; background:${C.deep}; position:relative; overflow:hidden; transition:transform .15s; }
  ...
  .sb-win-badge { display:inline-block; background:rgba(255,215,0,.15); border:1px solid rgba(255,215,0,.35); color:${C.gold}; font-size:.65rem; font-weight:700; letter-spacing:.06em; padding:1px 7px; border-radius:10px; margin-top:4px; }
  ...
  .sb-rank-badge { display:inline-flex; align-items:center; gap:4px; font-family:'Bebas Neue',sans-serif; font-size:.75rem; letter-spacing:.06em; padding:2px 8px; border-radius:10px; margin-top:5px; }
  ...
  .tab { flex-shrink:0; padding:7px 15px; border-radius:6px; border:1px solid ${C.border}; background:${C.card}; color:${C.muted}; font-family:'Rajdhani',sans-serif; font-size:.82rem; font-weight:600; letter-spacing:.05em; cursor:pointer; transition:all .15s; }
  .tab:hover { border-color:${C.accent}; color:${C.accent}; }
  .tab.active { background:linear-gradient(135deg,rgba(0,212,255,.14),rgba(168,85,247,.14)); border-color:${C.accent}; color:${C.accent}; }
  .card { background:${C.card}; border:1px solid ${C.border}; border-radius:10px; padding:18px; margin-bottom:14px; }
  .card-title { font-family:'Bebas Neue',sans-serif; font-size:1.22rem; letter-spacing:.08em; color:${C.accent}; margin-bottom:14px; display:flex; align-items:center; gap:8px; flex-wrap:wrap; }
  ...
  input, select { width:100%; background:${C.deep}; border:1px solid ${C.border}; border-radius:6px; padding:7px 9px; color:${C.text}; font-family:'Rajdhani',sans-serif; font-size:.95rem; outline:none; transition:border-color .15s; }
  input:focus, select:focus { border-color:${C.accent}; }
  select option { background:${C.card}; }
  .btn { padding:8px 16px; border-radius:6px; border:none; font-family:'Rajdhani',sans-serif; font-size:.85rem; font-weight:700; letter-spacing:.07em; cursor:pointer; transition:all .15s; }
  ...
  .add-row { width:100%; padding:9px; border-radius:7px; border:1px dashed ${C.border}; background:transparent; color:${C.muted}; font-family:'Rajdhani',sans-serif; font-size:.85rem; cursor:pointer; transition:all .15s; margin-top:8px; }
  .add-row:hover { border-color:${C.accent}; color:${C.accent}; }
  .tally { display:inline-flex; align-items:center; border:1px solid ${C.border}; border-radius:7px; overflow:hidden; background:${C.deep}; }
  .tally-btn { border:none; background:transparent; color:${C.muted}; font-size:1rem; font-weight:700; width:28px; height:28px; cursor:pointer; transition:all .12s; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
  ...
  .team-placement-row { display:flex; align-items:center; gap:12px; flex-wrap:wrap; padding:10px 12px; border-radius:8px; margin-bottom:6px; }
  ...
  .placement-result-badge { display:inline-flex; align-items:center; gap:5px; padding:4px 12px; border-radius:20px; font-family:'Bebas Neue',sans-serif; font-size:.95rem; letter-spacing:.04em; flex-shrink:0; }
  .match-wrap { overflow-x:auto; }
  .match-table { width:100%; border-collapse:collapse; min-width:480px; }
  .match-table th { font-size:.65rem; text-transform:uppercase; letter-spacing:.1em; color:${C.muted}; text-align:center; padding:7px 4px; border-bottom:1px solid ${C.border}; white-space:nowrap; }
  ...
  .team-sep td { padding:6px 10px; font-family:'Bebas Neue',sans-serif; letter-spacing:.08em; font-size:.88rem; border-bottom:1px solid rgba(168,85,247,.25) !important; background:rgba(168,85,247,.05); }
  ...
  .lb-row { display:grid; grid-template-columns:42px 1fr auto 62px; gap:6px; align-items:center; padding:10px 14px; border-radius:8px; margin-bottom:5px; border:1px solid ${C.border}; background:${C.deep}; transition:transform .13s; }
  ...
  .match-pip { font-size:.62rem; font-weight:700; padding:1px 6px; border-radius:4px; letter-spacing:.03em; white-space:nowrap; }
  ...
  @media(max-width:520px){ .rules-grid{grid-template-columns:1fr} .lb-row{grid-template-columns:36px 1fr auto 52px} .tally-btn{width:26px;height:26px;font-size:.9rem} }
`;

// ─── Sub-components ───────────────────────────────────────────────────────────
function Tally({ value, onChange, color }) {
  const v = parseInt(value) || 0;
  const cls = `tally-val${color==="water"?" water":v>0?" nonzero":""}`;
  return (
    <div className="tally">
...
const GAME_DOC = "tournament/game1";

// ─── Main App ─────────────────────────────────────────────────────────────────
export default function App() {
  const [tab,    setTab]    = useState(0);
  const [teams,  setTeamsLocal] = useState(DEFAULT_TEAMS);
  const [synced, setSynced] = useState(false);
  const [saving, setSaving] = useState(false);

  // ── Firebase: listen for real-time updates ─────────────────────────────────
  useEffect(() => {
    const ref = doc(db, GAME_DOC);
    const unsub = onSnapshot(ref, (snap) => {
      if (snap.exists()) {
        const data = snap.data();
        if (data.teams) setTeamsLocal(data.teams);
      }
      setSynced(true);
    });
    return () => unsub();
  ...
  const setTeams = (updater) => {
    setTeamsLocal(prev => {
      ...
      saveTimeout[0] = setTimeout(async () => {
        setSaving(true);
        try {
          await setDoc(doc(db, GAME_DOC), { teams: next }, { merge: true });
  ...
  const updateTeamMatchPlacement = (tid, mi, val) =>
    setTeams(ts => ts.map(t => {
      ...
      const mp = [...t.matchPlacements]; mp[mi] = val;
  ...
  const updateStat = (tid, pid, mi, field, val) =>
    setTeams(ts => ts.map(t => {
      if (t.id!==tid) return t;
      return {...t, members: t.members.map(m => {
  ...
  const teamsWithStats = teams.map((t,ti) => ({
    ...t, color:teamColor(ti), wins:teamWins(t), ...calcTeam(t),
  }));
  const allPlayers = teams.flatMap((t,ti) =>
    t.members.filter(m=>m.name.trim()).map(m => ({
      ...m, teamName:t.name, teamId:t.id, teamColor:teamColor(ti), ...calcPlayer(m)
    }))
  );
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="hero">
          ...
          <div className="sync-bar" style={{color:C.muted}}>
            <div className={`sync-dot ${synced?"sync-live":"sync-off"}`}/>
            {saving ? "Saving..." : synced ? "Live · All screens synced" : "Connecting..."}
          </div>
        </div>
        ...
        <div className="scoreboard">
          ...
          <div className="sb-teams">
            {teamsWithStats.map((t,i) => {
              ...
              const leading = rank===1 && t.pts>0;
              return (
                <div className="sb-team" key={t.id} style={{border:`1px solid ${leading?t.color+"88":t.color+"44"}`}}>
                  ...
                  <div className="sb-team-sub">💧 {t.shots} shots · {t.members.filter(m=>m.name.trim()).length} players</div>
                  <div className={`sb-rank-badge ${leading?"sb-rank-1":"sb-rank-n"}`}>
                    {leading ? "👑 #1 LEADING" : rank ? `#${rank}` : "—"}
                  </div>
                  <div className="match-strip" style={{marginTop:5}}>
                    {t.matchPlacements.map((p,mi)=><MatchPip key={mi} placement={p}/>)}
                  </div>
                  {t.wins>0 && <div className="sb-win-badge">🏆 {t.wins} WIN{t.wins>1?"S":""}</div>}
        ...
        {tab===0 && (
          <>
              ...
              <div className="team-block" key={team.id} style={{borderColor:teamColor(ti)+"55"}}>
                <div className="team-header" style={{borderBottomColor:teamColor(ti)+"44"}}>
                  <div className="team-name-wrap">
                    ...
                    <input value={team.name} placeholder={`Team ${ti+1}`}
                      onChange={e=>updateTeamName(team.id, e.target.value)}
                      style={{borderColor:teamColor(ti)+"66"}}/>
                  </div>
                  <button className="btn btn-ghost" style={{padding:"6px 12px",fontSize:".78rem",flexShrink:0}}
                    onClick={()=>removeTeam(team.id)}>🗑 Remove Team</button>
                </div>
                {team.members.map((m,mi)=>(
                  <div className="member-row" key={m.id}>
                    <span className="member-num" style={{color:teamColor(ti)}}>{mi+1}</span>
                    <input value={m.name} placeholder={`Player ${mi+1}`}
                      onChange={e=>updateMemberName(team.id,m.id,e.target.value)}/>
                    <button className="btn btn-ghost" style={{padding:"5px 9px",fontSize:".85rem",flexShrink:0}}
                      onClick={()=>removeMember(team.id,m.id)}>✕</button>
                  </div>
                ))}
            ...
            <button className="add-row" style={{marginBottom:18}} onClick={addTeam}>+ Add New Team</button>
            <div className="card">
              <div className="card-title">📋 Rules & Scoring</div>
              <div className="rules-grid">
                {[
                  ["👑","Victory Royale","+10 pts"],["🔥","Top 5","+7 pts"],["⬆️","Top 10","+5 pts"],
                  ["🎯","Elimination","+1 pt"],["🔄","Reboot","+1 pt"],["💥","Team Wipe","+2 pts"],
                  ["💧","Early Death","2 shots"],["💧","Mid-game Death","1 shot"],
                  ["💧","First Knocked","+1 shot"],["🚫","No Rage Quitting","Commissioner"],
                ].map(([icon,rule,val])=>(
        ...
        {tab>=1 && tab<=5 && (
          <div className="card">
            <div className="card-title">
              🎮 Match {tab} Scorecard
              <span className="score-pill" style={{marginLeft:"auto"}}>{scoredCount}/{allPlayers.length} entered</span>
            </div>
            {allPlayers.length===0
              ? <p style={{color:C.muted,textAlign:"center",padding:"24px 0"}}>Add players in the Roster tab first.</p>
              : (
                <div className="match-wrap">
                  <table className="match-table">
                    <thead>
                      <tr>
                        <th>Player</th><th>Placement</th><th>Elims</th>
                        <th>Reboots</th><th>💥 Wipes</th><th>💧 Shots</th><th>Pts</th>
                      </tr>
                    </thead>
                    <tbody>
                      {teams.map((team,ti)=>{
                        ...
                        return (
                          <Fragment key={team.id}>
                            <tr className="team-sep">
                              <td colSpan={7} style={{padding:"8px 10px"}}>
                                <div className="team-placement-row" style={{background:`${tc}0d`,border:`1px solid ${tc}33`}}>
                                  <span className="team-placement-label" style={{color:tc}}>▸ {team.name||`Team ${ti+1}`}</span>
                                  {(()=>{
                                    const r=teamRankMap[team.id];
                                    const lead=r===1&&(teamsWithStats.find(t=>t.id===team.id)?.pts||0)>0;
                                    return r ? <span className={`sb-rank-badge ${lead?"sb-rank-1":"sb-rank-n"}`} style={{fontSize:".65rem"}}>{lead?"👑 #1":`#${r}`}</span> : null;
                                  })()}
                                  <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                                    <label className="inline" style={{color:C.muted,fontSize:".65rem"}}>TEAM PLACEMENT</label>
                                    <select className="team-placement-sel" value={teamPlace}
                                      onChange={e=>updateTeamMatchPlacement(team.id,matchIdx,e.target.value)}
                                      style={{borderColor:teamPlace?tc+"88":C.border,color:teamPlace?C.text:C.muted}}>
                                      <option value="">— Not set —</option>
                                      {Array.from({length:20},(_,n)=>n+1).map(n=>(
                                        <option value={n} key={n}>{n===1?"👑 #1 — Victory Royale":n<=5?`🔥 #${n} — Top 5`:`#${n}`}</option>
                                      ))}
                                    </select>
                                  </div>
                                  {teamPlace && (
                                    <span className="placement-result-badge"
                                      style={isWin
                            ...
                            {visible.map(member=>{
                              ...
                              const placePts = m.placement!==""?PLACEMENT_PTS(m.placement):0;
                              const rowPts = placePts+(parseInt(m.elims)||0)*ELIM_PTS+(parseInt(m.reboots)||0)*REBOOT_PTS+(parseInt(m.teamWipes)||0)*TEAM_WIPE_PTS;
                              const hasData = rowPts>0||(parseInt(m.waterShots)||0)>0;
                              return (
                                <tr key={member.id}>
                                  <td><div className="player-cell">{member.name}</div></td>
                                  <td>
                                    <select className="placement-sel" value={m.placement}
                                      onChange={e=>updateStat(team.id,member.id,matchIdx,"placement",e.target.value)}>
                                      <option value="">—</option>
                                      {Array.from({length:30},(_,n)=>n+1).map(n=>(
                                        <option value={n} key={n}>{n===1?"👑 #1":n<=5?`🔥 #${n}`:`#${n}`}</option>
                                      ))}
                                    </select>
                                  </td>
                                  <td style={{textAlign:"center"}}><Tally value={m.elims}      onChange={v=>updateStat(team.id,member.id,matchIdx,"elims",v)}/></td>
                                  <td style={{textAlign:"center"}}><Tally value={m.reboots}    onChange={v=>updateStat(team.id,member.id,matchIdx,"reboots",v)}/></td>
                                  <td style={{textAlign:"center"}}><Tally value={m.teamWipes}  onChange={v=>updateStat(team.id,member.id,matchIdx,"teamWipes",v)}/></td>
                                  <td style={{textAlign:"center"}}><Tally value={m.waterShots} color="water" onChange={v=>updateStat(team.id,member.id,matchIdx,"waterShots",v)}/></td>
                            ...
                            {(()=>{
                              ...
                              const memberPts = visible.reduce((sum,member)=>{
                                const m=member.matches[matchIdx];
                                return sum+(m.placement!==""?PLACEMENT_PTS(m.placement):0)+(parseInt(m.elims)||0)*ELIM_PTS+(parseInt(m.reboots)||0)*REBOOT_PTS+(parseInt(m.teamWipes)||0)*TEAM_WIPE_PTS;
                              },0);
                              const total = placePts+memberPts;
                              return (
                                <tr>
                                  <td colSpan={6} style={{textAlign:"right",paddingRight:8,color:C.muted,fontSize:".72rem",fontWeight:600,borderTop:`1px solid ${tc}33`}}>
                                    MATCH TOTAL {placePts>0&&<span style={{color:tc,marginLeft:6}}>+{placePts} placement</span>}
                                  </td>
                                  <td style={{textAlign:"center",borderTop:`1px solid ${tc}33`}}>
                                    <span className="pts-badge" style={{background:`${tc}22`,color:tc,border:`1px solid ${tc}66`,fontSize:".9rem"}}>{total}</span>
                                  </td>
                                </tr>
                              );
                            })()}
        ...
        {tab===6 && (
          <>
                    ...
                    <div className="lb-row" key={team.id} style={{borderColor:team.color+"44"}}>
                      ...
                      <div>
                        ...
                        <div className="match-strip" style={{marginTop:4}}>
                          {team.matchPlacements.map((p,mi)=><MatchPip key={mi} placement={p}/>)}
                        </div>
                      </div>
            ...
            <div className="card">
              ...
              <div className="rules-grid">
                {(()=>{
                  const topTeam=teamLeaderboard[0], topPlayer=playerLeaderboard[0];
